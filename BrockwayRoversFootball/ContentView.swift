...
struct BundledImage: View {
    ...
    var body: some View {
        Group {
            if let url = Bundle.main.url(forResource: name, withExtension: ext), let image = UIImage(contentsOfFile: url.path) { Image(uiImage: image).resizable() }
            else { Rectangle().fill(Color.black) }
        }
    }
}
...
struct PageHeader: View {
    ...
    var body: some View {
        ...
        }.frame(maxWidth: .infinity, alignment: .leading).padding(.horizontal, 18).padding(.top, 16).padding(.bottom, 12)
    }
}
            ...
            VStack(spacing: 0) {
                ZStack(alignment: .bottomLeading) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack(spacing: 10) {
                            BundledImage(name: "rover-icon-source", ext: "png").scaledToFit().frame(width: 46, height: 46).clipShape(Circle())
                ...
                VStack(spacing: 14) {
                    VStack(alignment: .leading, spacing: 14) {
                        HStack { Text("NEXT GAME").font(.caption.weight(.black)).tracking(1.5).foregroundStyle(RoverTheme.red); Spacer(); Text("FRI • SEPT 4").font(.caption.weight(.bold)).foregroundStyle(RoverTheme.muted) }
                        HStack {
                        ...
                        Link(destination: liveStreamURL) { HStack { Image(systemName: "play.rectangle.fill"); Text("WATCH LIVE").font(.subheadline.weight(.black)); Spacer(); Image(systemName: "arrow.up.right") }.foregroundStyle(.white).padding(.horizontal, 16).frame(height: 50).background(RoverTheme.red).clipShape(RoundedRectangle(cornerRadius: 14)) }
                    }.card()
                    ...
                    VStack(alignment: .leading, spacing: 13) { Text("COMING UP").font(.headline.weight(.black)); CompactGame(date: "SEP 4", opponent: "@ DuBois", time: "7:00 PM"); Divider(); CompactGame(date: "SEP 11", opponent: "St. Marys", time: "7:00 PM"); Divider(); CompactGame(date: "SEP 18", opponent: "Port Allegany", time: "7:00 PM") }.card()
...
struct MoreView: View {
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                PageHeader(title: "Brockway Rovers", subtitle: "2026 FOOTBALL")
                    .padding(.horizontal, -18)
                Link(destination: liveStreamURL) {
                    HStack {
                        Image(systemName: "play.rectangle.fill").foregroundStyle(RoverTheme.red)
                        Text("Watch Rovers Live").font(.headline).foregroundStyle(.white)
